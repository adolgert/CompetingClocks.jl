...
\item $t_0$---Absolute time at which the transition is enabled.

\item $t_e$---Absolute time at which to place the zero-point of the distribution. If $t_e < t_0$, it's equivalent to saying that we are cutting off the left side of the distribution, up to $t=t_0-t_e$. If $t_e>t_0$, it's equivalent to saying the transition cannot possibly fire until after time absolute time $t_e$.
...
Let's start with the simplifying assumption that $t_e=t_0$. Use Eq.~\ref{eqn:basesample} to write sampling in absolute time.
\begin{equation}
	u = S(\tau - t_0)
\end{equation}
...
\begin{equation}
u=\exp\left(-\int_{t_0}^{\tau}\lambda(s-t_0)ds\right)\label{eqn:exponentialform}
\end{equation}
...
\begin{eqnarray}
	u&=&\exp\left(-\int_{t_0}^{\tau}\lambda_0(s-t_0)ds\right) \\
	&=& \exp\left(-\int_{t_0}^{t_n}\lambda_0(s-t_0)ds\right)\exp\left(-\int_{t_n}^{\tau'}\lambda_{n}(s-t_0)ds\right)\label{eqn:expequivalence}
\end{eqnarray}
...
\begin{equation}
	\ln u=-\int_{t_0}^{\tau}\lambda_0(s-t_0)ds = -\int_{t_0}^{t_n}\lambda_0(s-t_0)ds - \int_{t_n}^{\tau'}\lambda_{n}(s-t_0)ds\label{eqn:logequivalence}
\end{equation}
...
\end{equation}
Multiplying both sides by the denominator shows a known rule about survival functions. The rule is that \emph{conditional survival is multiplicative.}
\begin{equation}
