...
Let ``X`` be an event path generated by a continuous-time stochastic system (SIR, assembly line, reliability, etc.). In CompetingClocks, a path is a sequence of (clock key, firing time) pairs:
