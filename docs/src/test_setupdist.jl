...
@testset "SetupRate Distribution" begin

    @testset "Constructor" begin
        # Valid construction
        d = SetupRate(2.0, 0.5; t0=1.0)
        @test d.α_max == 2.0
        @test d.k_rem == 0.5
        @test d.t0 == 1.0

        # Default t0
        d_default = SetupRate(2.0, 0.5)
        @test d_default.t0 == 0.0

        # Invalid parameters
        @test_throws ErrorException SetupRate(-1.0, 0.5)
        @test_throws ErrorException SetupRate(2.0, -0.5)
        @test_throws ErrorException SetupRate(2.0, 0.5; t0=-1.0)
    end

    @testset "Parameter extraction" begin
        d = SetupRate(3.0, 0.8; t0=2.5)
        p = params(d)
        @test p == (3.0, 0.8, 2.5)
    end
    ...
    @testset "Different parameter regimes" begin
        ...
        d_slow = SetupRate(1.0, 0.1; t0=0.0)
        @test pdf(d_slow, 0.5) < pdf(d_slow, 5.0)

        # Large k_rem (fast approach to α_max)
        d_fast = SetupRate(1.0, 10.0; t0=0.0)
        @test pdf(d_fast, 0.5) > pdf(d_fast, 5.0)

        # Large α_max
        d_high_rate = SetupRate(100.0, 1.0; t0=0.0)
        @test cdf(d_high_rate, 0.5) > cdf(SetupRate(1.0, 1.0; t0=0.0), 0.5)
    end
    ...
    @testset "Sequential fabrication events" begin
        # Simulate the intended use case: sequential fabrication events
        Random.seed!(789)
        α_max = 2.0
        k_rem = 0.5

        # First fabrication starts at t0=0
        d1 = SetupRate(α_max, k_rem; t0=0.0)
        t1 = rand(d1)
        @test t1 >= 0.0

        # Second fabrication starts with t0=t1 (continuing the hazard ramp-up)
        d2 = SetupRate(α_max, k_rem; t0=t1)
        Δt2 = rand(d2)  # Time from t1 to t2
        t2 = t1 + Δt2
        @test Δt2 >= 0.0

        # Third fabrication
        d3 = SetupRate(α_max, k_rem; t0=t2)
        Δt3 = rand(d3)
        t3 = t2 + Δt3
        @test Δt3 >= 0.0
        ...
        @test λ1_start == 0.0  # First event starts with no fabrication activity
