...
sgp = plot(x1, y1, label="Shifted Gamma(5,1)")
x = 0:0.01:15
y = pdf.(Gamma(5.0, 1.0), x)
plot!(sgp, x, y, label="Gamma(5,1)")
vline!(sgp, [t], label="New start time")
xlabel!(sgp, "Time")
ylabel!(sgp, "Probability Density Function")
title!(sgp, "Change to PDF After Not Firing")
DisplayAs.PNG(sgp)
...
indp = plot(a, b, layout=(1, 2), size=(1000, 400))
DisplayAs.PNG(indp)
...
condp = plot(bb, bp, size=(1000, 400))
DisplayAs.PNG(condp)
# 
...
DisplayAs.PNG(plot(a, b, size=(1000, 400)))
