# Tests for the explicit re-evaluation coupling verb (Milestone 5, guarantee G6):
# `reenable!(sampler, key, dist, te, when, coupling)` and its context wrapper. A
# still-enabled clock whose distribution changes mid-flight must let the CALLER
# name the pathwise coupling — :carry (deterministic, IPA-safe) or :redraw
# (redraw-at-change) — rather than have the backend pick silently. Plain @testset
# (not @safetestset) so the file-scope oracles below are shared.
...
# age τ under `coupling`, then read the firing time. Returns the firing time.
function drive_single(B, λ0, λ1, τ, coupling, seed)
    s = B{Symbol,Float64}(seed)
    enable!(s, :A, Exponential(1 / λ0), 0.0, 0.0)
    t, _ = next(s, 0.0)
    t < τ && return t                      # fired before the change, old law
    reenable!(s, :A, Exponential(1 / λ1), 0.0, τ, coupling)  # te=0 keeps the age
    t2, _ = next(s, τ)
    return t2
end
...
# B is constant-rate λB. If nothing fires before τ, re-evaluate A at age τ under
# `coupling` and continue. Returns (winner_is_A, firing_time).
function drive_race(B, λA0, λA1, λB, τ, coupling, seed)
    s = B{Symbol,Float64}(seed)
    enable!(s, :A, Exponential(1 / λA0), 0.0, 0.0)
    enable!(s, :B, Exponential(1 / λB), 0.0, 0.0)
    t, w = next(s, 0.0)
    t < τ && return (w === :A, t)
    reenable!(s, :A, Exponential(1 / λA1), 0.0, τ, coupling)
    t2, w2 = next(s, τ)
    return (w2 === :A, t2)
end
...
@testset "reenable: an unsupported coupling symbol and a :carry request on a non-carry backend both throw an informative error, and reenable! on a clock that is not enabled is rejected" begin
    ftf = FirstToFire{Symbol,Float64}()
    enable!(ftf, :x, Exponential(1.0), 0.0, 0.0)
    @test_throws ArgumentError reenable!(ftf, :x, Exponential(2.0), 0.0, 0.5, :bogus)
    # Not-enabled clock.
    @test_throws ArgumentError reenable!(ftf, :missing, Exponential(1.0), 0.0, 0.5, :redraw)

    # :carry on an exponential-only backend fails at the boundary, naming the trait.
    dc = DirectCall{Symbol,Float64}()
    enable!(dc, :x, Exponential(1.0), 0.0, 0.0)
    @test_throws ArgumentError reenable!(dc, :x, Exponential(2.0), 0.0, 0.5, :carry)
    # ...but :redraw is fine — it just replaces the memoryless rate.
    reenable!(dc, :x, Exponential(0.25), 0.0, 0.5, :redraw)
    @test dc[:x] == rate(Exponential(0.25))
end


@testset "reenable: :carry with an unchanged distribution leaves the schedule bit-for-bit unchanged while :redraw generally moves it, on both CombinedNextReaction and FirstToFire" begin
    ...
    for B in (CombinedNextReaction, FirstToFire)
        ...
        s = B{Symbol,Float64}(2024)
        enable!(s, :A, dist, 0.0, 0.0)
        sched0 = s[:A]
        reenable!(s, :A, dist, 0.0, 0.4, :carry)
        @test s[:A] == sched0                       # carry + same dist = identity

        s2 = B{Symbol,Float64}(2024)
        enable!(s2, :A, dist, 0.0, 0.0)
        sched0b = s2[:A]
        reenable!(s2, :A, dist, 0.0, 0.4, :redraw)
...
@testset "reenable: under :carry the new schedule is a deterministic, continuous function of a small rate change (moves O(epsilon)), while :redraw jumps, on CombinedNextReaction and FirstToFire" begin
    ...
    for B in (CombinedNextReaction, FirstToFire)
        ...
        function carried_sched(newrate)
            s = B{Symbol,Float64}(777)
            enable!(s, :A, Exponential(1 / 0.8), 0.0, 0.0)  # rate 0.8
            reenable!(s, :A, Exponential(1 / newrate), 0.0, τ, :carry)
            return s[:A]
        end
        ...
        reenable!(s, :A, Exponential(1 / 1.5), 0.0, τ, :redraw)
        @test s[:A] != base
    end
end


@testset "reenable: for exponential old and new distributions, :carry reduces EXACTLY to Gibson-Bruck rate-ratio scaling of the remaining time to relative tolerance 1e-12, on CombinedNextReaction and FirstToFire" begin
...
@testset "reenable: FirstReaction accepts either coupling symbol and treats both as the moot redraw-everything re-evaluation, replacing the stored distribution" begin
    # FirstReaction retains no schedule and redraws at every next, so :carry and
    # :redraw resolve to the same act — replace the distribution. Both are
    # accepted so a driver can issue one reenable! call against any backend.
    for coupling in (:carry, :redraw)
        s = FirstReaction{Symbol,Float64}(99)
        enable!(s, :A, Exponential(2.0), 0.0, 0.0)
        reenable!(s, :A, Gamma(2.0, 0.5), 0.0, 0.5, coupling)
        @test isenabled(s, :A)
...
@testset "reenable: on a single clock with a piecewise-exponential hazard, both couplings on both scheduling backends and FirstReaction reproduce the analytic piecewise firing-time mean within 4 standard errors, over 40,000 replications" begin
    λ0, λ1, τ = 0.6, 1.8, 0.7
    ...
    for (label, B, coupling) in cases
        times = Vector{Float64}(undef, nrep)
        for r in 1:nrep
            times[r] = drive_single(B, λ0, λ1, τ, coupling, 100_000 + r)
        end
...
@testset "reenable: in a two-clock race with a deterministic mid-flight re-evaluation of one clock, :carry and :redraw on CombinedNextReaction agree with each other and with FirstReaction on the winner probability and mean firing time, within 4 pooled standard errors, over 40,000 replications" begin
    λA0, λA1, λB, τ = 0.5, 2.0, 1.0, 0.6
    nrep = 40_000
    function run_case(B, coupling, base)
        winA = Vector{Float64}(undef, nrep)
        times = Vector{Float64}(undef, nrep)
        for r in 1:nrep
            a, t = drive_race(B, λA0, λA1, λB, τ, coupling, base + r)
            winA[r] = a ? 1.0 : 0.0
            times[r] = t
        end
...
@testset "reenable: a context-level reenable! keeping the clock's age routes the segment boundary to the path-likelihood watcher, so the accumulated path log-likelihood equals the hand-computed piecewise value" begin
    ...
    reenable!(ctx, :A, dist_new, -τ, :carry)   # keep age: te stays 0
    fire!(ctx, :A, tfire)
...
@testset "reenable: a context-level reenable! updates the TrajectoryRecorder's live table so the fired clock's back-calculated uniform uses the CURRENT segment's distribution, not the retired one" begin
    ...
    # be computed against the retired distribution.
    ...
    enable!(ctx, :trigger, Exponential(1.0))
    fire!(ctx, :trigger, τ)
    reenable!(ctx, :A, dist_new, -τ, :redraw)   # keep age; redraw the schedule
